...
const STATUS_FILTERS = ['All', 'Active', 'Completed', 'Cancelled', 'Disputed']
const SORT_OPTIONS = [
...
const getStatusTone = (status) => {
  ...
  if (normalized === 'completed') return { bg: '#ECFDF5', fg: '#047857', border: '#A7F3D0' }
  if (normalized === 'active') return { bg: '#EFF6FF', fg: '#1D4ED8', border: '#BFDBFE' }
  if (normalized === 'cancelled') return { bg: '#F3F4F6', fg: '#6B7280', border: '#E5E7EB' }
...
const JobSeekerJobReports = ({ navigation }) => {
  ...
  const dummyJobs = useMemo(
    () => [
      { id: 'js-jr-001', jobTitle: 'Night Shift Cleaners', recruiterName: 'BuildRight Pty Ltd', status: 'Completed', dateISO: '2026-01-10T19:00:00+11:00', hoursWorked: 8, gross: 360, fee: 18, net: 342 },
      { id: 'js-jr-002', jobTitle: 'Warehouse Inventory Support', recruiterName: 'Northside Logistics', status: 'Active', dateISO: '2026-01-09T08:20:00+11:00', hoursWorked: 4, gross: 180, fee: 9, net: 171 },
      { id: 'js-jr-003', jobTitle: 'Commercial Painting · Emergency Shift', recruiterName: 'PrimeCo Builders', status: 'Disputed', dateISO: '2026-01-05T07:40:00+11:00', hoursWorked: 12, gross: 540, fee: 27, net: 513 },
      { id: 'js-jr-004', jobTitle: 'Event Setup Crew', recruiterName: 'City Events AU', status: 'Completed', dateISO: '2026-01-03T16:10:00+11:00', hoursWorked: 6, gross: 240, fee: 12, net: 228 },
      { id: 'js-jr-005', jobTitle: 'Café Barista Cover', recruiterName: 'Brew & Co.', status: 'Cancelled', dateISO: '2025-12-28T06:00:00+11:00', hoursWorked: 0, gross: 0, fee: 0, net: 0 },
      { id: 'js-jr-006', jobTitle: 'Forklift Operator', recruiterName: 'Dockside Warehouse', status: 'Completed', dateISO: '2025-12-22T09:30:00+11:00', hoursWorked: 10, gross: 420, fee: 21, net: 399 },
      { id: 'js-jr-007', jobTitle: 'Hospitality Floor Staff', recruiterName: 'Harbour Venue Group', status: 'Completed', dateISO: '2025-12-12T18:30:00+11:00', hoursWorked: 7.5, gross: 315, fee: 15.75, net: 299.25 },
      { id: 'js-jr-008', jobTitle: 'Event Pack-down Crew', recruiterName: 'City Events AU', status: 'Completed', dateISO: '2025-12-06T23:20:00+11:00', hoursWorked: 5, gross: 190, fee: 9.5, net: 180.5 },
      { id: 'js-jr-009', jobTitle: 'Delivery Route Support', recruiterName: 'Metro Couriers', status: 'Active', dateISO: '2025-12-09T07:10:00+11:00', hoursWorked: 3, gross: 120, fee: 6, net: 114 },
      { id: 'js-jr-010', jobTitle: 'High-rise Window Cleaning', recruiterName: 'ClearView Services', status: 'Completed', dateISO: '2025-11-28T06:30:00+11:00', hoursWorked: 9, gross: 405, fee: 20.25, net: 384.75 },
      { id: 'js-jr-011', jobTitle: 'Retail Stock Replenishment', recruiterName: 'Central Retail', status: 'Completed', dateISO: '2025-11-18T05:40:00+11:00', hoursWorked: 6, gross: 240, fee: 12, net: 228 },
      { id: 'js-jr-012', jobTitle: 'Kitchen Hand', recruiterName: 'GreenFork Kitchen', status: 'Cancelled', dateISO: '2025-12-02T17:15:00+11:00', hoursWorked: 0, gross: 0, fee: 0, net: 0 },
      { id: 'js-jr-013', jobTitle: 'Office Move Assistance', recruiterName: 'Northside Offices', status: 'Completed', dateISO: '2025-10-09T11:55:00+11:00', hoursWorked: 8, gross: 320, fee: 16, net: 304 },
      { id: 'js-jr-014', jobTitle: 'Warehouse Pick & Pack', recruiterName: 'Dockside Warehouse', status: 'Completed', dateISO: '2025-10-28T12:05:00+11:00', hoursWorked: 7, gross: 280, fee: 14, net: 266 },
      { id: 'js-jr-015', jobTitle: 'Site Safety Marshal', recruiterName: 'PrimeCo Builders', status: 'Disputed', dateISO: '2025-12-31T07:00:00+11:00', hoursWorked: 4, gross: 200, fee: 10, net: 190 },
    ],
    [],
  )
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    const filtered = dummyJobs
      .filter((item) => isInRange(item.dateISO, from, to))
      .filter((item) => (status === 'All' ? true : item.status === status))

    const sorted = [...filtered]
    if (sortKey === 'highest_earnings') {
      sorted.sort((a, b) => Number(b.net || 0) - Number(a.net || 0))
    } else if (sortKey === 'highest_hours') {
      sorted.sort((a, b) => Number(b.hoursWorked || 0) - Number(a.hoursWorked || 0))
    } else {
      sorted.sort((a, b) => new Date(b.dateISO).getTime() - new Date(a.dateISO).getTime())
    }

    return sorted
  }, [activeRange, dummyJobs, sortKey, statusFilter])

  const kpis = useMemo(() => {
    const totalJobs = filteredAndSorted.length
    const completedJobs = filteredAndSorted.filter((x) => x.status === 'Completed').length
    const totalHours = filteredAndSorted.reduce((sum, x) => sum + Number(x.hoursWorked || 0), 0)
    const totalNet = filteredAndSorted.reduce((sum, x) => sum + Number(x.net || 0), 0)

    const statusCounts = {
      Active: filteredAndSorted.filter((x) => x.status === 'Active').length,
      Completed: filteredAndSorted.filter((x) => x.status === 'Completed').length,
      Cancelled: filteredAndSorted.filter((x) => x.status === 'Cancelled').length,
      Disputed: filteredAndSorted.filter((x) => x.status === 'Disputed').length,
    }

    return { totalJobs, completedJobs, totalHours, totalNet, statusCounts }
  }, [filteredAndSorted])
  ...
  const exportToCSV = async () => {
    try {
      ...
      const headers = 'Date,Job,Recruiter,Status,Hours,Gross,Fee,Net\n'
      const rows = filteredAndSorted
        .map((r) =>
          [
            ...
            escapeCSV(r.status),
            escapeCSV(Number(r.hoursWorked || 0).toString()),
      ...
      const fileName = `jobseeker_job_reports_${new Date().toISOString().split('T')[0]}.csv`
  ...
  const exportToPDF = async () => {
    try {
      ...
      page.drawText(
        `Jobs: ${kpis.totalJobs}   Completed: ${kpis.completedJobs}   Hours: ${kpis.totalHours.toFixed(1)}   Net: ${formatCurrency(kpis.totalNet)}`,
      ...
      page.drawText('Status', { x: width - margin - 160, y, size: fontSize, font: boldFont })
      page.drawText('Net', { x: width - margin - 70, y, size: fontSize, font: boldFont })
      y -= 10
      ...
      filteredAndSorted.forEach((r) => {
        ...
        page.drawText(String(r.jobTitle || '').slice(0, 30), { x: margin + 70, y, size: fontSize, font })
        page.drawText(String(r.status || ''), { x: width - margin - 160, y, size: fontSize, font })
        page.drawText(formatCurrency(r.net), { x: width - margin - 70, y, size: fontSize, font })
        y -= lineHeight
      })

      const pdfBytes = await pdfDoc.save()
      const fileName = `jobseeker_job_reports_${new Date().toISOString().split('T')[0]}.pdf`
      const baseDir = Platform.OS === 'android' ? RNFS.CachesDirectoryPath : RNFS.DocumentDirectoryPath
      const path = `${baseDir}/${fileName}`
      ...
      <View style={styles.rowCard}>
        <View style={styles.rowHeader}>
          <View style={styles.rowTitleCol}>
            ...
            <AppText variant={Variant.caption} style={styles.rowMeta}>
              {item.recruiterName} · {formatShortDate(item.dateISO)} · {Number(item.hoursWorked || 0).toString()}h
            </AppText>
          </View>
        ...
        <View style={styles.rowFooter}>
          <AppText variant={Variant.caption} style={styles.amountMeta}>
            Gross {formatCurrency(item.gross)} · Fee {formatCurrency(item.fee)}
          </AppText>
  ...
  return (
    <View style={styles.container}>
      <AppHeader
        ...
        gradientColors={['#0EA5E9', '#22C55E']}
        children={
          <View style={styles.headerSub}>
            <AppText variant={Variant.bodySmall} style={styles.headerSubText}>
              Review jobs, hours, and earnings by period.
            </AppText>
          </View>
        }
      />
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.summaryGrid}>
            {renderSummaryCard({ title: 'Total jobs', value: String(kpis.totalJobs), icon: 'briefcase-outline', tone: 'info' })}
            {renderSummaryCard({ title: 'Completed', value: String(kpis.completedJobs), icon: 'checkmark-circle-outline', tone: 'success' })}
            {renderSummaryCard({ title: 'Total hours', value: kpis.totalHours.toFixed(1), icon: 'time-outline', tone: 'primary' })}
            {renderSummaryCard({ title: 'Total earnings (net)', value: formatCurrency(kpis.totalNet), icon: 'cash-outline', tone: 'warning' })}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.breakdownRow}>
            {Object.entries(kpis.statusCounts).map(([key, value]) => (
              <View key={key} style={styles.breakdownChip}>
                <AppText variant={Variant.caption} style={styles.breakdownLabel}>
                  {key}
                </AppText>
                <AppText variant={Variant.bodyMedium} style={styles.breakdownValue}>
                  {String(value)}
                </AppText>
              </View>
            ))}
...
export default JobSeekerJobReports
...
const styles = StyleSheet.create({
  ...
  breakdownRow: { flexDirection: 'row', flexWrap: 'wrap', gap: wp(2), marginTop: hp(1.2) },
  breakdownChip: {
    backgroundColor: colors.grayE8 || '#F3F4F6',
    paddingHorizontal: wp(3),
    paddingVertical: hp(1),
  ...
  breakdownLabel: { color: colors.gray || '#6B7280' },
  breakdownValue: { color: colors.secondary, fontWeight: '800', marginTop: hp(0.3) },
  ...
  amountMeta: { color: colors.gray || '#6B7280', flex: 1, marginRight: wp(2) },
  netAmount: { color: colors.secondary, fontWeight: '900' },
